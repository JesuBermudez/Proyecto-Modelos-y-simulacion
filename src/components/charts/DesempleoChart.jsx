...
export default function DesempleoChart() {
  ...
  useEffect(() => {
    axios
      .get(
        "https://api-node-motor-finaciero-production.up.railway.app/api/macro/desempleo/colombia"
      )
      .then((response) => {
        let apiData = response.data.unemploymentData;
        ...
        if (apiData != undefined) {
          ...
          let TransformedData = apiData.map((u) => {
            return {
              time: new Date(u.year_month).getTime() / 1000,
  ...
  const handleButtonClick = (e) => {
    ...
    if (!lineAdded) {
      ...
      axios
        .get(
          `https://mpf.fly.dev/predicciones/macroeconomicas/desempleo/${months}`
        )
        .then((response) => {
          const container =
